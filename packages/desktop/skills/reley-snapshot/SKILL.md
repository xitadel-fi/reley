---
name: reley-snapshot
description: Save the current sandbox state and restore it later. Optionally include the captured program-version pins on restore. Fork a snapshot into a fresh sandbox.
---
...
- `reley-versions` — managing the underlying program versions.
- `reley-tx-template` — re-running a frozen ix against restored state.
- `reley-troubleshooting` — "Snapshot restore brings back old program ELFs".
