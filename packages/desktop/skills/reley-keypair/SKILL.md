---
name: reley-keypair
description: Dev signing keys for the sandbox — generate or import, airdrop SOL to them, use as the payer or as an additional signer in tx templates and workflows.
---

# Dev keypairs

Keypairs in Reley are **project-scoped** — they live under the project
folder, not globally. They travel with the project (copy folder = copy
keypairs).

## File location

`<projectRoot>/.reley/keypairs/keypairs.json`
...
- `reley-tx-template` — `isSigner` flags + run-time signer params.
- `reley-workflow` — `additionalSignerKeypairIds` on tx steps.
- `reley-patch` — `setField mintAuthority → <your keypair's pubkey>` so
