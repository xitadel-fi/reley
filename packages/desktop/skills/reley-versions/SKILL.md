---
name: reley-versions
description: Test upgrade and downgrade paths by keeping multiple ELF versions of a program. Switch the active version, pin per sandbox or per workflow step, and diff IDLs + behaviour across versions.
---

# Program versions

Reley tracks many versions of each program so you can test forward / backward
...
Per-version IDLs live at `.reley/idls/<programId>__<versionId>.json` (the
default `.reley/idls/<programId>.json` is the fallback when no per-version
IDL is attached).
...
IPC equivalents (see `@reley/shared/src/ipc/methods.ts`):
...
step, even on throw. See `reley-workflow`.
...
the pin. See `reley-tests` for full upgrade/downgrade testcase shape.
...
- **Run-compare panel**: pick program + left/right versions + a tx template,
  Reley pins left, runs, pins right, runs, returns side-by-side
...
the active one, Reley picks the most-recently created remaining version as
the new active.

## Cross-references

- `reley-snapshot` — snapshot v2 captures + optionally restores version pins.
- `reley-workflow` — per-step pin field + `setProgramVersion` persistent flip.
- `reley-tests` — multi-case test suites; canonical upgrade/downgrade testcase.
- `reley-tx-template` — `programVersionOverrides` at run time.
- `reley-account` — IDL fallback chain for decoding.
