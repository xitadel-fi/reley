---
name: reley-troubleshooting
description: Common errors and fixes — mint authority mismatches on cloned mainnet mints, missing signers, version-pin desync, snapshot restore failures, replay divergence.
---
...
See `reley-patch` for the setField op.
...
Check:
1. `~/Library/Application Support/Reley/worker-trace.log` (macOS) — last
   trace line tells you where boot stopped.
2. `.reley.json` exists at the project root and has `formatVersion: 2`.
3. `.reley/programs/`, `.reley/tx-templates/`, etc. exist (Reley creates
   them on first save; missing dirs are auto-created).

If `formatVersion: 1`, Reley auto-migrates on load. If migration fails,
delete `.reley.json` and re-create the project from Welcome.
...
Open `~/Library/Application Support/Reley/worker-trace.log`. The last line
...
- `reley-tx-template` — instruction ordering + run-time signer params.
- `reley-workflow` — per-step pin / multi-signer fields.
- `reley-versions` — manage + diff program versions.
- `reley-snapshot` — capture / restore including version metadata.
- `reley-patch` — fixing cloned-state mismatches.
