---
name: reley-workflow
description: Chain a sequence of transactions, airdrops, time warps, and resets. Halts on first failed tx. Each step can pin a program version and add extra signers.
---
...
`<projectRoot>/.reley/workflows/<id>.json` — one per workflow.
...
| Field | Meaning |
|---|---|
| `programId` | Base58 program id (must exist in project `.reley/programs/`) |
...
1. Read `.reley/tx-templates/<templateId>.json`
...
- **Halt-on-fail**: if a step throws or a tx step's send fails, no subsequent
  ...
  must not halt on failed tx, use `reley-tests` instead.
...
- `reley-tx-template` — instruction format.
- `reley-tests` — assertion-driven multi-case sibling; no halt on tx fail.
- `reley-versions` — manage program versions before pinning.
- `reley-keypair` — add signer keypairs.
- `reley-sandbox` — what sandbox the workflow runs against.
- `reley-troubleshooting` — debugging failed steps.
